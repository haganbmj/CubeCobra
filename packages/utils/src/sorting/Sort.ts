...
export function getColorIdentity(colors: string[]): string {
  if (colors.length === 0) {
    ...
    if (color && Object.keys(COLOR_MAP).includes(color)) {
...
export function getColorCombination(colors: string[]): string {
  ...
  if (colors.length === 2) {
    return GUILD_MAP[ordered] || 'Unknown';
  }
  if (colors.length === 3) {
    return SHARD_AND_WEDGE_MAP[ordered] || 'Unknown';
  }
  if (colors.length === 4) {
    return FOUR_COLOR_MAP[ordered] || 'Unknown';
  }
  return 'Five Color';
}
...
const normalizeCollectorNumberForSorting = (card: Card): string => {
  ...
  if (lastCharacter && isNaN(parseInt(lastCharacter, 10))) {
...
export const SortFunctionsOnDetails = (sort: string) => (a: any, b: any) => {
  ...
  return sortFunction ? sortFunction({ details: a }, { details: b }) : 0;
};
...
function priceBucketLabel(index: number, prefix: string): string {
  if (index === 0) {
    return `< ${prefix}${priceBuckets[0] || 0}`;
  }
  if (index === priceBuckets.length) {
    return `>= ${prefix}${priceBuckets[priceBuckets.length - 1] || 0}`;
  }
  return `${prefix}${priceBuckets[index - 1] || 0} - ${prefix}${(priceBuckets[index] || 0) - 0.01}`;
}

function priceBucketIndex(price: number): number {
  if (price < (priceBuckets[0] || 0)) {
    return 0;
  }
  for (let i = 1; i < priceBuckets.length; i++) {
    if (price >= (priceBuckets[i - 1] || 0) && price < (priceBuckets[i] || 0)) {
      return i;
    }
  }
...
function wordCountBucket(wordCount: number): string {
  ...
  for (let i = 2; i < wordCountBuckets.length; i++) {
    if (wordCount < (wordCountBuckets[i] || 0)) {
      return `${wordCountBuckets[i - 1] || 0}+`;
    }
  }
  return '100+';
}
...
function accumulateCards(groups: DeepSorted, accumulator: Card[]): void {
