...
function downloadDefaultCards() {
  console.log('Downloading card database from Scryfall.com');
...
function convertParsedCost(card, isExtra) {
  if (isExtra) {
    ...
    } else if (card.card_faces[0].colors || card.card_faces[0].mana_cost) {
    ...
    if (parsed_cost) {
      ...
      console.error(`Unhandled convertParsedCost: ${card.name}, ${card.set_name}`)
    }
    return parsed_cost;
  }
}
...
function convertColors(card, isExtra) {
  ...
  if (isExtra) {
    ...
    // } else if (card.card_faces && card.card_faces[0].colors) {
    ...
    console.error(`Unhandled convertColors: ${card.name}, ${card.set_name}`);
  }
}
...
function convertType(card, isExtra) {
  if (isExtra) {
    ...
    if (card.type_line.includes('//')) {
      _type = card.type_line.substring(0, card.type_line.indexOf('/')).trim();
    } else {
